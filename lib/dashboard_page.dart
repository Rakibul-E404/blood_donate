...
class UserInfo {
  ...
  UserInfo({required this.label, required this.value});
}

class ProfileWidget extends StatelessWidget {
  final List<UserInfo> personalInfo = [
    UserInfo(label: "Email", value: "user@example.com"),
    UserInfo(label: "Mobile Number", value: "1234567890"),
    UserInfo(label: "Blood Group", value: "O+"),
          ...
          Row(
            ...
            children: [
              DropdownButton<UserInfo>(
                hint: const Text("Select Personal Information"),
                value: null, // You can set a default value if needed
                onChanged: (UserInfo? newValue) {
                  if (newValue != null) {
                    print("Selected ${newValue.label}: ${newValue.value}");
                  }
                },
                items: personalInfo.map((UserInfo info) {
